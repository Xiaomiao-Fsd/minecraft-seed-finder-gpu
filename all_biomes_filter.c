...
static void usage(const char *argv0) {
    fprintf(stderr,
        ...
        "  --mc newest|1_21     cubiomes version approximation (default newest)\n"
...
int main(int argc, char **argv) {
    ...
    fprintf(out, "seed,required_biome_count,check_return,mc_approx,radius_blocks,scale\n");
    ...
    while (fgets(line, sizeof(line), in)) {
        ...
        if (has_all_biomes((uint64_t)s, mc, radius, scale, required, required_len, &ret)) {
            fprintf(out, "%" PRId64 ",%d,%d,%s,%d,%d\n", s, required_len, ret, mc_name, radius, scale);
            hits++;
        }
        processed++;
